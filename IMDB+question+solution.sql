...
WITH actor_summary AS (
    SELECT N.NAME AS actor_name, SUM(R.total_votes) AS total_votes, COUNT(DISTINCT R.movie_id) AS movie_count, ROUND(SUM(R.avg_rating * R.total_votes) / SUM(R.total_votes), 2) AS actor_avg_rating
    FROM movie AS M
    INNER JOIN (  SELECT movie_id,  avg_rating, COUNT(*) AS total_votes FROM ratings 
        GROUP BY movie_id
